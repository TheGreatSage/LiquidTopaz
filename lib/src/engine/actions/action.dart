library liquid.engine.actions.action;
...
abstract class Action {
  ...
  void addAction(Action action, [Actor actor]) {
    action.bind(actor == null ? _actor : actor);
    _actions.add(action);
  }

  void addEvent(EventType type, {Actor actor, other,
  Vec pos, Direction dir}) {
    _gameResult.events.add(new Event(type, actor, pos, dir, other));
  }
...
class ActionResult {
  ...
  const ActionResult.alternate(this.alternative): succeeded = false, done = true;
}